...
public class ClientSessionManager extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) {
        ...
        */
        ...
        oPacket.EncodeShort(Configuration.GetConfig().MAPLE_VERSION);
        oPacket.EncodeString(Configuration.GetConfig().BUILD_VERSION);
        oPacket.EncodeInt(RecvSeq);
        oPacket.EncodeInt(SendSeq);
        oPacket.EncodeByte(Configuration.GetConfig().SERVER_TYPE);
    ...
    public void channelInactive(ChannelHandlerContext ctx) {
        ...
        if (pClient.nWorldID > 0 && pClient.pAccount != null) { //Set account state to 0 on auth server.
            CenterSocket pSocket = CenterSessionManager.GetByWorldID(pClient.nWorldID);
            if (pSocket != null) {
