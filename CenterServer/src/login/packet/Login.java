...
import user.character.inventory.GW_ItemSlotEquip;
...
public class Login {
    ...
    public static void OnBan(LoginServerSocket aThis, InPacket iPacket) {
        AuthServerSocket pSocket = AuthServerSessionManager.pSession;
        if (pSocket == null) {
            return;
        }
        int nAccountID = -1;
        ...
        pSocket.SendPacket(Auth.Ban(nType, sReason, nAccountID, sIP, sMAC, sHWID, uBanDuration));
    }
        ...
        try {
            ...
            if (UserStorage.GetStorage().mAccountStorage.containsKey(nSessionID)) {
                ...
                if (pAccount.nAccountID == nAccountID) {
                    pAuthSocket.SendPacket(Auth.SetSPW(nAccountID, sSPW));
                }
            }
        } finally {
            UserStorage.GetStorage().Unlock();
