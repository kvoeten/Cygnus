...
public class RankingWorker extends Thread {
    ...
    public void run() {
        try {
            Thread.sleep(1000);
            ProcessRanking();
            if (AuthServerSessionManager.pSession != null) {
                SendWorldCharacterInfo(AuthServerSessionManager.pSession);
            }
        ...
        try (Connection con = Database.GetConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    if (IsElligableForRanking(con, dwCharacterID)) {
    ...
    private static boolean IsElligableForRanking(Connection con, int dwCharacterID) throws SQLException {
    ...
    public static void SendWorldCharacterInfo(AuthServerSocket pSocket) {
        try (Connection con = Database.GetConnection();
            ...
            while (rs.next()) {
                ...
                pSocket.SendPacket(Auth.OnSendAvatarData(pAvatar));
            }
        } catch (SQLException ex) {
            Logger.getLogger(RankingWorker.class.getName()).log(Level.SEVERE, null, ex);
        }
