...
public class GW_CharacterStat {
    ...
    public void SaveNew(Connection con) {
        try (PreparedStatement ps = con.prepareStatement("INSERT INTO GW_CharacterStat (dwCharacterID, dwCharacterIDForLog, dwWorldIDForLog, "
                ...
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");) {
            Database.Excecute(con, ps, dwCharacterID, dwCharacterIDForLog, dwWorldIDForLog, sCharacterName, nGender, nSkin, nFace, nHair, 
                    nMixBaseHairColor, nMixAddHairColor, nMixHairBaseProb, nLevel, nJob, nSTR, nDEX, nINT, nLUK, nHP, nMHP, nMP, nMMP, nAP, 
                    Arrays.toString(aSP).replace("[", "").replace("]", "").replace(" ", ""), nExp64, nPop, nWP, dwPosMap, nPortal, nSubJob, 
                    nDefFaceAcc, nFatigue, nLastFatigureUpdateTime, nCharismaEXP, nInsightExp, nWillExp, nCraftExp, nSenseExp, nCharmExp, 
                    DayLimit, nPvPExp, nPVPGrade, nPvpPoint, nPvpModeLevel, nPvpModeType, nEventPoint, ftLastLogoutTimeHigh, 
                    ftLastLogoutTimeLow, bBurning);
        } catch (SQLException ex) {
            Logger.getLogger(GW_CharacterStat.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static boolean IsResistanceArchor(int nJob) {
        return nJob / 100 == 33;
    }
