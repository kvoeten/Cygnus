...
public class AvatarLook {
    ...
    public void SaveNew(Connection con) {
        try (PreparedStatement ps = con.prepareStatement("INSERT INTO AvatarLook (dwCharacterID, nGender, nSkin, nFace, nHair, "
                ...
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");) {
            Database.Excecute(con, ps, dwCharacterID, nGender, nSkin, nFace, nHair, nJob, nWeaponsStickerID, nWeaponID, nSubWeaponID,
                    bDrawElfEar, nXenonDefFaceAcc, nDemonSlayerDefFaceAcc, nBeastDefFaceAcc, nBeastEars, nBeastTail, nMixedHairColor, nMixHairPercent);
        } catch (SQLException ex) {
            Logger.getLogger(AvatarLook.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public static AvatarLook Load(Connection c, int dwCharacterID) {
    ...
    public void Encode(OutPacket oPacket, ZeroInfo pZero) {
        ...
        oPacket.EncodeInt(pZero == null ? anHairEquip[ItemSlotIndex.BP_WEAPON] : pZero.nLapis);
