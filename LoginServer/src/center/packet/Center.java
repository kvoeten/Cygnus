...
public class Center {
    ...
    public static void Ban(ClientSocket pSocket, int nType, String sReason, long uBanDuration) {
        ...
        Date pBanEndDate = new Date(System.currentTimeMillis() + uBanDuration);
        APIFactory.mIPBan.put(pSocket.GetIP(), pBanEndDate);

        if (nType == 2) { //Instantly put it in the blocklist instead of waiting for the blocklist update.
            APIFactory.mMACBan.put(pSocket.pAccount.sMAC, pBanEndDate);
            APIFactory.mHWIDBan.put(pSocket.pAccount.sHWID, pBanEndDate);
        }

        //TODO: CENTER HANDLER, AUTH HANDLER
        if (pCenterSocket != null) {
            OutPacket oPacket = new OutPacket(LoopBackPacket.Ban);
            oPacket.EncodeLong(pSocket.nSessionID);
            oPacket.EncodeInt(nType);
            oPacket.EncodeString(sReason);
            oPacket.EncodeLong(uBanDuration);
            switch (nType) {
                case 0:
                    oPacket.EncodeString(pSocket.GetIP());
                    break;
                case 1:
                    oPacket.EncodeInt(pSocket.pAccount.nAccountID);
                    break;
                case 2:
                    oPacket.EncodeInt(pSocket.pAccount.nAccountID);
                    oPacket.EncodeString(pSocket.GetIP());
                ...
                case 3:
                    oPacket.EncodeString(pSocket.GetIP());
                    oPacket.EncodeString(pSocket.pAccount.sMAC);
                    oPacket.EncodeString(pSocket.pAccount.sHWID);
                    break;
                case 4:
                    oPacket.EncodeInt(pSocket.pAccount.nAccountID);
                    oPacket.EncodeString(pSocket.GetIP());
            ...
            pCenterSocket.SendPacket(oPacket);
        }
